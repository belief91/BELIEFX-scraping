...
async function scraperCalendrierBC() {
  ...
  $("tr").each((_, row) => {
    ...
    const actualText = $(row).find("td.calendar-item").eq(0).text().trim();
    const previousCell = $(row).find("td.calendar-item").eq(1).clone();
    previousCell.find('[id="revised"]').remove();
    const previousText = previousCell.text().trim();
    const consensusText = $(row).find("td.calendar-item").eq(2).text().trim();
    const forecastText = $(row).find("td.calendar-item").eq(3).text().trim();
