...
async function scraperCalendrierBC() {
  ...
  $("tr").each((_, row) => {
    ...
    const actualText = $(row).find("#actual").first().text().trim();
    const previousText = $(row).find("#previous").first().text().trim();
    const consensusText = $(row).find("#consensus").first().text().trim();
    const forecastText = $(row).find("#forecast").first().text().trim();
